...
const RegistroVenda = () => {
  ...
  const [formData, setFormData] = useState({ produtoId: '', quantidade: 1, precoTotal: '0.00' });
  ...
  useEffect(() => {
    const fetchProdutos = async () => {
      console.log('Nome da Empresa do contexto:', nomeEmpresa);
      try {
  ...
  const handleGerarComprovante = async () => {
    console.log('Nome da Empresa no momento da geração:', nomeEmpresa);
          ...
          <Box sx={{
            ...
            <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end' }}>
              ...
              <Button
                ...
                disabled={!clienteNome || !validateCPF(clienteCpf)}
              >
                Gerar Comprovante
              </Button>
            </Box>
